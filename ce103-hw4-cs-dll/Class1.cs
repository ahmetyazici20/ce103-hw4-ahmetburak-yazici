...
namespace ce103_hw4_cs_dll
{
    public class Class1
    {
		public int hw4_fibonacciNumber_cs(int fiIndex)
		{
		...
		public string hw4_strrev_cs(string fiStr)
		{
			string rvrsdStr = "";
		...
		public int hw4_strlen_cs(string fiStr)
		{
			int len = 0;
		...
		public string hw4_strcat_cs(string fiDest, string fiSrc)
		{
			return fiDest + fiSrc;
		}
		public int hw4_strcmp_cs(string fiLhs, string fiRhs)
		{
			int i1 = 0, i2 = 0;
		...
		public string hw4_strcpy_cs(string foDestination, string fiSource)
		{

			foDestination = fiSource;
			return foDestination;
		}
		public void hw4_hex2bin_cs(string fiHex, int fiHexLen, byte[] foBin)
		{
			//char[] foBinArr = new char[fiHexLen / 2];

			int[] count = { 0, 0 };  // counts for each ascii and hex value of selected element 
			...
			{  // array for assigning foBin elements
				...
				for (int b = 0; b < 2; b++)
				{  // for loop for calculating values of ascii summation
					temp[b] = (char)fiHex[a + b];  // assigning first fiHex element to temp
					if (b == 0)
					{  // if b == 0 means the first element
					  
						if (temp[b] >= '0' && temp[b] <= '9')
						{  
							count[b] = temp[b] - '0';  // find int value of count
							count[b] *= 16;  
						}
						else if (temp[b] >= 'A' && temp[b] <= 'F')
						{  
							count[b] = temp[b] - 'A' + 10;  // take the difference between temp[b] and 'A' then plus 10
							count[b] *= 16;
						}
					}
					else
		...
		public void hw4_bin2hex_cs(byte[] fiBin, int fiBinLen, char[] foHex)
		{
